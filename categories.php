    ...
    <main class="home_content">
      <div class="mainHeader">
        <div class="mainHeaderLeft">
          <p class="main_title">Lisää kategori</p>
        ...
        <div class="dataContent sellerDataContent" style="display: flex">
          <div class="sellersLeft" style="flex: 1; flex-direction: column">
            <div class="sellerHeaderTitle">
              <p class="sellerTitle">Lisää Kategori</p>
            </div>
            <div class="sellerContent">
              <input type="text" placeholder="Kategorin nimi (Ammattirakentaja yms.)" id="categoryInput">
            </div>
            <div class="sellerBottom">
              <button class="btn" id="addCategory">Lisää</button>
            </div>
          </div>

          <div class="sellersRight" style="flex: 1">
            <div class="sellerHeaderTitle">
                <p class="sellerTitle">Kategorit</p>
              </div>
            <table style="height: 100%">
              <thead>
                <tr class="productTableHeadRow">
                  <th>#</th>
                  <th>Kategori</th>
                  <th>Tuotteiden määrä</th>
                  <th>Asiakkaiden määrä</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
              ...
              $sql->execute();
              $categoriesResults = $sql->fetchAll(PDO::FETCH_OBJ);

              $count = 1;
              foreach ($categoriesResults as $category) {

                // Count this category's products number
                $productSql = $conn->prepare("SELECT * FROM products WHERE product_category = :product_category");
                $productSql->execute(array(
                  ':product_category' => $category->category_id
                ));
                $productsCount = $productSql->rowCount();

                // Count this category's clients number
              ...
              ?>
                <tr class="productTableDataRow">
                  ...
                  <td>340</td>
                  <td><i class="bx bxs-edit-alt" data-id="<?php echo $category->category_id;?>"></i></td>
                </tr>
              <?php
                $count++;
              }
